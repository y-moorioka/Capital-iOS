...
final class ActionsCollectionViewCell: UICollectionViewCell {
    ...
    @IBAction private func actionSend() {
        if let actionsViewModel = actionsViewModel {
            notifyCheck()
            if !notifyAuth {
                self.delegate?.showAlert(title: L10n.Notify.confirmTitle, message: L10n.Notify.confirmDescription(Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as! String), actions: [(L10n.Notify.confirmNo, .cancel), (L10n.Notify.confirmYes, .default)], mode: 0, completion: { Int in
                    if Int == 0 {
